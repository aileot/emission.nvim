...
local function create_autocmds_21()
  ...
  for _, event in ipairs(cache.config.highlight.additional_recache_events) do
    ...
    vim.api.nvim_create_autocmd(event, {group = group, callback = _42_})
  end
  local function _43_(_241)
    return request_to_attach_buf_21(_241.buf)
  end
  vim.api.nvim_create_autocmd("BufEnter", {group = group, callback = _43_})
  local function _44_(_241)
    return request_to_detach_buf_21(_241.buf)
  end
  vim.api.nvim_create_autocmd("BufLeave", {group = group, callback = _44_})
